...
#### 2.2.1 AbstractBasicRag（RAG 核心流程）
...
public abstract class AbstractBasicRag implements RagFlow {
    ...
    @Override
    public RagAnswer answer(String userId, String question, List<String> sources) {
        // 1. 加载记忆上下文
        MemoryContext memoryContext = loadMemoryContext(userId, question);
        
        // 2. Query 增强（指代消解）
        String enhancedQuery = enhanceQueryWithMemory(question, memoryContext);
        
        // 3. 复杂度分类
        ComplexityLevelEnum complexity = classifyComplexity(enhancedQuery);
        
        // 4. 根据复杂度路由
        switch (complexity) {
            case SIMPLE:
                return handleSimpleQuestion(enhancedQuery, memoryContext);
            case MODERATE:
                return handleModerateQuestion(enhancedQuery, memoryContext, sources);
            case COMPLEX:
                return handleComplexQuestion(enhancedQuery, memoryContext, sources);
        }
    }
}
```
...
**钩子方法**（子类实现）:
- `shouldUseShortTermMemory()`: 是否使用短期记忆
- `shouldUseLongTermMemory()`: 是否使用长期记忆
- `shouldEnableEvaluation()`: 是否启用质量评估
- `buildPromptWithMemories()`: 构建带记忆的 Prompt

---

#### 2.2.2 ShortTermMemoryManager（短期记忆管理）
...
#### 2.2.3 LongTermMemoryManager（长期记忆管理）
