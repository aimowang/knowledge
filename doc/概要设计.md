...
│  (Services, RAG Pipeline, Orchestrator) │
└──────────────┬──────────────────────────┘
               │
┌──────────────▼──────────────────────────┐
│         Data Access Layer               │
...
### 3.3 RAG 核心架构（管道+策略模式）

本次重构采用**管道模式 + 策略模式 + 编排器模式**，将原本 910 行的复杂类拆分为职责清晰的组件。

#### 3.3.1 管道模式（Pipeline Pattern）

RAG 流程被拆分为多个独立阶段，按顺序执行：
...
| **core** | 核心业务逻辑（管道+策略+编排器） | Spring AI, Resilience4j |
...
graph TB
    A[用户请求] --> B[JWT 认证]
    B --> C[RagOrchestrator.beforeExecute]
    C --> D[缓存检查]
    D -->|命中| E[返回缓存答案]
    D -->|未命中| F[加载短期/长期记忆]
    F --> G[分类问题复杂度]
    G --> H[RagPipeline.execute]
    H --> I[QueryPreprocessingStage]
    I --> J[RetrievalStage]
    J --> K[PostProcessingStage]
    K --> L[GenerationStage]
    L --> M[RagOrchestrator.afterExecute]
    M --> N[保存短期记忆]
    N --> O[异步质量评估]
    O --> P[提取长期记忆]
    P --> Q[缓存结果]
...
| 阶段 | 职责 | 关键操作 |
|------|------|----------|
| **QueryPreprocessingStage** | 查询预处理 | 去除空格、规范化空白字符 |
| **RetrievalStage** | 文档检索 | 智能选择查询、向量检索、来源过滤 |
| **PostProcessingStage** | 后处理 | 去重 → 过滤 → 重排序 → 压缩 |
| **GenerationStage** | 答案生成 | 构建 Prompt、调用 LLM、提取引用 |
...
| 维度 | 重构前 | 重构后 | 改进 |
|------|--------|--------|------|
| **核心类行数** | AbstractBasicRag: 910行 | AbstractRagFlow: 98行 | **-89%** ⭐ |
...
**上下文对象（2个）**
- `RagContext` - RAG 执行上下文（193行）
- `MemoryContext` - 记忆上下文（65行）

**管道框架（6个）**
- `RagPipeline` - 管道接口（47行）
- `PipelineStage` - 阶段接口（40行）
- `DefaultRagPipeline` - 默认管道实现（128行）
- `QueryPreprocessingStage` - 查询预处理（59行）
- `RetrievalStage` - 文档检索（88行）
- `PostProcessingStage` - 后处理（87行）
- `GenerationStage` - 答案生成（187行）

**策略组件（9个）**
- `QueryEnhancementStrategy` - 查询增强接口（33行）
- `RetrievalStrategy` - 检索策略接口（36行）
- `DocumentProcessingStrategy` - 文档处理接口（45行）
- `MemoryBasedQueryEnhancer` - 基于记忆的指代消解（114行）
- `KeywordExpansionEnhancer` - 关键词扩展（95行）
- `MultiQueryGenerator` - 多查询生成器（108行）
- `DeduplicationStrategy` - 智能去重（123行）
- `FilteringStrategy` - 质量过滤（47行）
- `CompressionStrategy` - 上下文压缩（64行）

**编排器（2个）**
- `RagOrchestrator` - 编排器接口（70行）
- `DefaultRagOrchestrator` - 默认编排器实现（262行）

**抽象基类和适配器（2个）**
- `AbstractRagFlow` - 新的抽象基类（98行）⭐ **从910行缩减到98行！**
- `LegacyRagFlowAdapter` - 向后兼容适配器（57行）

**总计：21个Java文件，约 2,000行高质量代码**
...
- **Pipeline Pattern**: 管道模式（责任链模式变体）
- **Strategy Pattern**: 策略模式（算法族封装）
- **Orchestrator Pattern**: 编排器模式（横切关注点协调）
